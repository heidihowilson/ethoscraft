...
function materialImpactKey(tgt: Entity): string {
  ...
  if (tgt.kind === 'mob') return MOBS[tgt.templateId]?.type === 'undead' ? 'impact_bone' : 'impact_flesh';
  return 'impact_flesh';
}
    ...
    if (target && target.kind !== 'object') {
      ...
      if (this.lastPortraitTarget !== target.id) {
        ...
        if (target.kind === 'player') {
          ...
          this.portraits.drawCrest(this.targetPortraitEl, crestIdForEntity(target.kind, MOBS[target.templateId]?.type));
        }
      }
