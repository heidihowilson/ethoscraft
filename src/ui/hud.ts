...
const SFX_MOB_FAMILIES = new Set(['animal', 'spider', 'murloc', 'kobold', 'humanoid', 'undead', 'troll', 'ogre', 'elemental', 'dragonkin', 'demon']);
...
function mobVoiceFamily(templateId: string): string | null {
  ...
  const fam = MOBS[templateId]?.mobType;
...
function materialImpactKey(tgt: Entity): string {
  ...
  if (tgt.kind === 'mob') return MOBS[tgt.templateId]?.mobType === 'undead' ? 'impact_bone' : 'impact_flesh';
  return 'impact_flesh';
}
    ...
    if (target && target.kind !== 'object') {
      ...
      if (this.lastPortraitTarget !== target.id) {
        ...
        if (target.kind === 'player') {
          ...
          this.portraits.drawCrest(this.targetPortraitEl, crestIdForEntity(target.kind, MOBS[target.templateId]?.mobType));
        }
      }
