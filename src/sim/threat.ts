...
//  - each point of modified healing output = 0.5 threat, including overheal,
...
export function threatModifier(source: Entity, school: string): number {
	...
	for (const a of source.auras) {
		...
		else if (a.kind === "righteous_fury" && school === "holy")
			mod *= RIGHTEOUS_FURY_THREAT_MULT;
	}
	return mod;
}

export function stealthDetectionMultiplier(
	observerLevel: number,
	stealthedLevel: number,
): number {
	const raw =
		STEALTH_DETECTION_MULT +
		(observerLevel - stealthedLevel) * STEALTH_DETECTION_PER_LEVEL;
	return Math.max(
		STEALTH_DETECTION_MIN_MULT,
		Math.min(STEALTH_DETECTION_MAX_MULT, raw),
	);
}
...
export function stealthDetectionRadius(
	...
	return (
		baseRadius * stealthDetectionMultiplier(observer.level, stealthed.level)
	);
}
